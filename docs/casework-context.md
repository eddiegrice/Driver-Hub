...
type CaseworkContextValue = {
  ...
  setAssignedAdmin: (ticketId: string, adminMemberId: string | null) => Promise<{ error: Error | null }>;
...
export function CaseworkProvider({ children }: { children: React.ReactNode }) {
  ...
  const setAssignedAdmin = useCallback(
    async (ticketId: string, adminMemberId: string | null) => {
      if (!supabase) return { error: new Error('No supabase') };
      const { error } = await updateCaseworkAssignedAdmin(supabase, ticketId, adminMemberId);
      if (!error) await refreshTickets();
      return { error };
    },
    [refreshTickets]
