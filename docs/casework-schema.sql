...
-- RLS admin checks use is_current_user_admin() so policies do not self-query public.members
-- under the same RLS stack (avoids recursion / broken member reads). See docs/casework-rls-fix-members-admin.sql.
