...
import { fetchTrafficStatus } from '@/lib/traffic-supabase';
import { FontSize, NeoGlass, NeoText, Radius, Spacing } from '@/constants/theme';
import type { TrafficStatus } from '@/types/traffic';
...
function statusLabel(value: string | null): string {
  ...
  if (v === 'freeflow' || v === 'free_flow') return 'Free flow';
  if (v === 'congested') return 'Congested';
  if (v === 'unknown') return 'Unknown';
  return value.replace(/_/g, ' ');
}

export default function TrafficFlowsScreen() {
  ...
  const load = useCallback(async () => {
    const { data: list, error: err } = await fetchTrafficStatus(supabase);
    setData(list);
    setError(err);
  }, []);
  ...
  if (isLoading && data.length === 0) {
    return (
      <View style={styles.screen}>
        <TabScreenHeader title="Traffic Flows" />
    ...
    <View style={styles.screen}>
      ...
      >
        <ThemedText style={styles.helper}>
          Traffic flow status (free flow / congested) for monitored routes.
        </ThemedText>
        ...
        {!error && data.length === 0 && (
          <GlassCard sleek borderRadius={Radius.lg} borderColor={LIGHT_EDGE} style={styles.emptyCard}>
            <ThemedText style={styles.empty}>No traffic flow data yet.</ThemedText>
            ...
            {filtered.map((row) => (
              ...
              >
                ...
                <View style={styles.row}>
                  ...
                  <ThemedText style={styles.value}>{statusLabel(row.trafficStatusValue)}</ThemedText>
                </View>
