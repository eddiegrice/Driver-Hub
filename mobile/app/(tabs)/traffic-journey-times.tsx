...
import { fetchTrafficTravelTimes } from '@/lib/traffic-supabase';
...
export default function JourneyTimesScreen() {
  const [data, setData] = useState<TrafficTravelTime[]>([]);
  ...
  const load = useCallback(async () => {
    const { data: list, error: err } = await fetchTrafficTravelTimes(supabase);
    setData(list);
    setError(err);
  }, []);
  ...
  if (isLoading && data.length === 0) {
    return (
      <View style={styles.screen}>
        <TabScreenHeader title="Journey Times" />
    ...
    <View style={styles.screen}>
      ...
      >
        <ThemedText style={styles.helper}>
          Live journey times for monitored routes.
        </ThemedText>
        ...
        {!error && data.length === 0 && (
          <GlassCard sleek borderRadius={Radius.lg} borderColor={LIGHT_EDGE} style={styles.emptyCard}>
            <ThemedText style={styles.empty}>No journey time data yet.</ThemedText>
          ...
          <View style={styles.list}>
            {filtered.map((row) => (
              ...
              >
                ...
                {row.direction && (
                  <ThemedText style={styles.meta} numberOfLines={1}>{row.direction}</ThemedText>
                )}
                <View style={styles.row}>
                  <ThemedText style={styles.label}>Travel time</ThemedText>
                  <ThemedText style={styles.value}>{formatDuration(row.travelTimeSec)}</ThemedText>
                </View>
                {row.freeFlowTravelTimeSec != null && (
                  <View style={styles.row}>
                    <ThemedText style={styles.label}>Free flow</ThemedText>
                    <ThemedText style={styles.value}>{formatDuration(row.freeFlowTravelTimeSec)}</ThemedText>
                  </View>
                )}
                <ThemedText style={styles.time}>Measured {formatMeasuredAt(row.measuredAt)}</ThemedText>
              </GlassCard>
            ))}
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  meta: { fontSize: FontSize.sm, color: NeoText.secondary, marginBottom: Spacing.xs },
  row: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: Spacing.xs },
  label: { fontSize: FontSize.sm, color: NeoText.muted },
  value: { fontSize: FontSize.body, fontWeight: '600', color: NeoText.primary },
