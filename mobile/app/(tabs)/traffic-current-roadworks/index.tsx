...
const SITUATION_TYPE = 'current_roadworks';
...
export default function CurrentRoadworksListScreen() {
    ...
    <View style={styles.screen}>
      ...
      >
        <ThemedText style={styles.helper}>
          Active roadworks on Glasgow roads. Tap for details.
        </ThemedText>
        ...
        {!error && list.length === 0 && (
          <GlassCard sleek borderRadius={Radius.lg} borderColor={LIGHT_EDGE} style={styles.emptyCard}>
            <ThemedText style={styles.empty}>No current roadworks.</ThemedText>
        ...
        {!error && filtered.length > 0 && (
          <View style={styles.list}>
            {filtered.map((s) => (
              <TouchableOpacity
                key={s.id}
                onPress={() => router.push(`/traffic-current-roadworks/${s.id}`)}
                activeOpacity={0.8}
              >
                <GlassCard sleek borderRadius={Radius.lg} borderColor={LIGHT_EDGE} contentStyle={styles.cardContent} style={styles.card}>
                  <ThemedText style={styles.title} numberOfLines={2}>{s.title ?? 'Roadworks'}</ThemedText>
