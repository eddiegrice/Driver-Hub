...
- **State / data on device:** AsyncStorage is still used for some local storage:
  - **AsyncStorage** keys: `@driverhub_member`, `@driverhub_news`, `@driverhub_news_seeded`, `@driverhub_polls`, `@driverhub_poll_responses`, `@driverhub_polls_seeded`, `@driverhub_poll_results_<pollId>` (casework is Supabase-only; no local casework key)
...
│       │   ├── casework/    # Stack: index (lists + create tiles), [id], create-internal, create-for-member
...
- **Casework:** List, new request (dropdown type, photos + documents, Supabase + Storage), ticket detail with Realtime thread, replies with attachments, request closure; schema `docs/casework-schema.md` / `.sql`.
...
- **Admin:** **News** and **casework** are implemented in-app (casework: Supabase + Storage + Realtime; push requires Edge Function + webhooks — see `docs/casework-push-setup.md`). **Polls / membership** admin screens are mostly **UI framework**. Verify **RLS** / run `docs/casework-schema.sql` in Supabase. Promote admins: `members.is_admin = true` in Supabase; default **false**.
...
- **Casework:** `useCasework()` — `createTicket({ type, subject, message, attachments })`, `addReply(ticketId, text, attachments?)`, `requestClosure`, admin: `setTicketStatus`, `setAssignedAdmin`, `createInternalCase`, `createCaseForMember`. Data: `lib/casework-supabase.ts`; SQL: `docs/casework-schema.sql`.
...
- **Routing:** `router.push('/')` (Home hub); `router.push('/scout')`, `router.push('/association')`; `router.push('/profile')`, `router.push('/chat')`, `router.push('/casework')`, `router.push('/news')`, `router.push('/polls')`, `router.push('/more')`; **`router.push('/admin')`**, **`/admin/news`**, **`/admin/news/create`**, **`/admin/news/${id}`**, **`/admin/casework`**, **`/admin/casework/${id}`**, **`/admin/casework/create-internal`**, **`/admin/casework/create-for-member`**, **`/admin/polls`**, **`/admin/membership`** (use `as Href` if TypeScript complains); `router.push('/events-gigs')`, `router.push('/events-sport')`, `router.push('/events-other')`; `router.push('/traffic-incidents')`, `router.push('/traffic-current-roadworks')`, `router.push('/traffic-future-roadworks')`, `router.push('/traffic-journey-times')`, `router.push('/traffic-flows')`, `router.push('/traffic-vms-signs')`, `router.push(\`/motorway-status/${code}\`)` (e.g. M8, M80, M74, M73); `router.push('/earnings-calc')`, `router.push('/library')`, `router.push('/docs-vault')`, `router.push('/member-e-card')`; `router.push(\`/casework/${id}\`)`, etc.
